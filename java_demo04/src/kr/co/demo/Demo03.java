...
public class Demo03 {
	...
	public static void main(String[] args) {
		 ...
		 * 		if 문의 경우 크기 비교 연산 등을 활용하여 특정 값의 범위에
		 * 		해당하는 경우를 선택할 수 있으나 switch 문의 경우 연산 결과에
		 * 		해당하는 값인 경우에만 선택적으로 실행되게 할 수 있다.
		 * 
		 * switch(연산식) {
		 * 		case 값1:
		 * 			실행 코드;
		 ...
		 * }
		 * 
		 * switch 문에서 default 는 생략 가능하며, case 에 해당하지 않는 값이
		 * 계산된 경우 실행된다.
		 * 
		 * switch 문의 각 case 안에는 break 구문을 사용하여 더 이상 switch 문이
		 * 동작하지 않고 종료되도록 만들어야 하며, 그렇지 않은 경우 특정 case 에
		 * 해당하여 실행한 이후 나머지 case 가 전부 동작한다.
		 */
