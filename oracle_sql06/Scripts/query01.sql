...
/* SET SERVEROUTPUT ON; 은 sqlplus 전용 명령어기 때문에 디비버에선 안됨 */
...
BEGIN
	SELECT EMPLOYEE_ID
	  ...
	  INTO pID, pNAME
	  FROM EMPLOYEES
	 WHERE EMPLOYEE_ID = :id; /*SQLPLUS에선 콜론 대신 & 사용*/
...
BEGIN
	NUM := :num;
	IF(NUM > 10) THEN
		DBMS_OUTPUT.PUT_LINE('NUM 값은 10보다 큽니다.');
	ELSIF(NUM = 10) THEN
		DBMS_OUTPUT.PUT_LINE('NUM 값은 10입니다.');
	ELSE
		DBMS_OUTPUT.PUT_LINE('NUM 값은 10보다 작습니다.');
	END IF;
END;
