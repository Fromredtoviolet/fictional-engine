...
<body>
	...
	Request 객체 : 요청에 대한 응답이 완전히 처리되기 전까지 유효하게 사용할 수 있다. 
	Request 객체 -> ${requestScope }
	해당 속성명으로 저장된 객체 반환 -> ${requestScope.속성명 }
	반환 받은 객체의 멤버 변수명으로 값 반환 -> ${requestScope.속성명.멤버변수명 }
	
	Session 객체 : 몇번의 요청이 발생하든 동일한 세션ID를 사용하여 요청을 할 때까지
				   유효하게 사용할 수 있다. (세션ID는 브라우저별로 제공)
	...
	Context 객체 : 서버 프로그램(톰캣 등)이 동작하는 동안 유효하게 사용할 수 있다.
				   일종의 전역속성이라고 볼 수 있다.
