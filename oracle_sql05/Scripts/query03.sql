 ...
 *  - 조회 속도는 빨라지지만 INSERT, UPDATE, DELETE 작업이 자주 이루어지는 경우
 *    해당 작업에 대한 성능 저하가 이루어진다.
 *  - INDEX 객체를 위한 추가 저장 공간이 필요하다.
 * 
 * 고유(UNIQUE) 인덱스
 * 	- PRIMARY KEY, UNIQUE 제약 조건을 생성하면 자동으로 설정함
 *  - 중복값이 포함될 수 없다.
 * 
 * 비고유(NONUNIQUE) 인덱스
...
/* 
 * 인덱스 재구성 : 
 * 테이블의 Record를 삭제해도 색인 정보는 남아 있기 때문에 이를 정리하기 위해 사용
 */
