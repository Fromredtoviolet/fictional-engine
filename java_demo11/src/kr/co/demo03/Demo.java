...
public class Demo {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * HashMap
		 * 		- 키 객체는 hashCode()와 equals()를 재정의해 등등 객체가 될 조건을
		 * 		  정해야 한다.
		 ...
		 * 		- 주로 프러퍼티 파일을 읽어 그 안의 속성을 프로그램에서 사용할 수 있게
		 * 		  만들기 위해 쓰인다.
		 * 		- 프로퍼티 파일은 주로 어플리케이션이 동작하는 과정에서 어플리케이션이 초기
		 * 		  동작에 필요한 값이나 설정값을 읽어서 사용할 수 있게 만들어져 있다.
		 * 		  (어플리케이션이 처음 동작할 때 데이터베이스 연결과 관련된 프로퍼티 파일을
		 * 		   읽어서 연결을 시도.)
		 */
		
		HashMap<String, Integer> aMap = new HashMap<String, Integer>();
		// 제네릭 타입을 <키, 값>으로 두 개 지정해야함
		...
		aMap.put("A", Integer.valueOf(50)); // 키는 중복 허용 안 함. 키가 중복되면 값이 수정됨.
		System.out.println(aMap); // 키만 별도로 수정할 수는 없다.
		...
		boolean isExists = aMap.containsKey("B");
		...
		for(Entry<String, Integer> e : aMap.entrySet()) {
			System.out.print("키:" + e.getKey());
			System.out.print(",값:" + e.getValue() + "\t");
		}
		System.out.println();
